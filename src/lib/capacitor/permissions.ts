...
class PermissionManager {
   ...
   */
  async checkPermissions(types: PermissionType[]): Promise<Record<PermissionType, PermissionResult>> {
   ...
   */
  async requestPermissions(types: PermissionType[]): Promise<Record<PermissionType, PermissionStatus>> {
  ...
  async openAppSettings(): Promise<void> {
    if (!this.isNative()) {
      console.warn('openAppSettings is only available on native platforms');
      return;
    }

    // Note: This requires a plugin like @capacitor/app
    // For now, just log a warning
    console.warn('Please enable permissions in your device settings');
  }
