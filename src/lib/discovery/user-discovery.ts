...
export class UserDiscoveryService {
  ...
  discover(options: UserDiscoveryOptions & { currentUserId?: string } = {}): UserDiscoveryResult {
    ...
    const effectiveLimit = Math.min(limit, MAX_LIMIT);
    const userContacts = currentUserId ? this.contacts.get(currentUserId) ?? new Set() : new Set<string>();
   ...
   */
  getUsersByDepartment(department: string, options: UserDiscoveryOptions = {}): UserDiscoveryResult {
