...
export class TransactionManager {
   ...
   */
  async getGasPrices(): Promise<TransactionManagerResult<{
    slow: string;
    standard: string;
    fast: string;
  }>> {
  ...
  async speedUpTransaction(
    ...
    if (newTx.maxFeePerGas) {
      ...
      newTx.maxFeePerGas = '0x' + BigInt(Math.floor(Number(currentMaxFee) * maxFeeMultiplier)).toString(16);

      if (newTx.maxPriorityFeePerGas) {
        const currentPriorityFee = BigInt(newTx.maxPriorityFeePerGas);
        newTx.maxPriorityFeePerGas = '0x' + BigInt(Math.floor(Number(currentPriorityFee) * multiplier)).toString(16);
      }
