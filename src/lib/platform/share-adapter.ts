...
export class CapacitorShareAdapter implements ShareAdapter {
  private getShare(): NonNullable<NonNullable<ShareWindow['Capacitor']>['Plugins']>['Share'] | null {
  ...
  async share(options: ShareOptions): Promise<ShareResult> {
    ...
    try {
      ...
      if (errorMessage.includes('cancelled') || errorMessage.includes('canceled') || errorMessage.includes('dismiss')) {
        return { success: false, cancelled: true };
      }
      return {
        success: false,
