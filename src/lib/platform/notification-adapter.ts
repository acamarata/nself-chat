...
export class CapacitorNotificationAdapter extends BaseNotificationAdapter {
  private getLocalNotifications(): NonNullable<NonNullable<NotificationWindow['Capacitor']>['Plugins']>['LocalNotifications'] | null {
    const win = typeof window !== 'undefined' ? (window as NotificationWindow) : null;
    return win?.Capacitor?.Plugins?.LocalNotifications ?? null;
  }

  private getBadge(): NonNullable<NonNullable<NotificationWindow['Capacitor']>['Plugins']>['Badge'] | null {
    const win = typeof window !== 'undefined' ? (window as NotificationWindow) : null;
        ...
        notifications: [
          {
            ...
            sound: options.sound === false ? undefined : (typeof options.sound === 'string' ? options.sound : undefined),
            extra: options.data,
            group: options.group,
            groupSummary: false,
