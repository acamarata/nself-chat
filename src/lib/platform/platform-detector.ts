...
export function isIOS(): boolean {
  ...
  return /iPhone|iPad|iPod/.test(userAgent) ||
         (platform === 'MacIntel' && win.navigator?.maxTouchPoints > 1); // iPad with desktop UA
}
...
export function getPlatformCapabilities(): PlatformCapabilities {
  ...
  // Check web APIs
  capabilities.pushNotifications = hasNotificationAPI() || platform === Platform.IOS || platform === Platform.ANDROID;
