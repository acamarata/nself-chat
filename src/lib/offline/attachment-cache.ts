...
export class AttachmentCache {
   ...
   */
  public async add(attachment: Omit<CachedAttachment, 'cachedAt' | 'lastAccessedAt' | 'accessCount'>): Promise<void> {
  ...
  private async evictLRU(bytesToFree: number): Promise<void> {
    ...
    console.log(`[AttachmentCache] Evicted ${toRemove.length} attachments (${freed} bytes)`);
  }
