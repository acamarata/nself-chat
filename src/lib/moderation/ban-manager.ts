...
export class BanManager {
  ...
  banUser(input: CreateBanInput): { success: boolean; ban?: Ban; errors?: string[] } {
    ...
    this.indexBan(ban);
    this.recordHistory(ban.id, 'created', input.moderatorId, input.moderatorName, null, ban.expiresAt, input.reason);

    // Also create IP ban if specified
    if (input.ipAddress && this.config.ipBansEnabled) {
      this.banIp(input.ipAddress, input.reason, input.moderatorId, input.moderatorName, input.durationMs);
    }
  ...
  updateBan(
    ...
    if (updates.durationMs !== undefined) {
      if (updates.durationMs === null) {
        ...
        if (updates.durationMs > this.config.maxBanDurationMs) {
          return { success: false, error: `Duration exceeds maximum of ${formatDuration(this.config.maxBanDurationMs)}` };
        }
    ...
    this.recordHistory(banId, action, updates.moderatorId, updates.moderatorName, previousExpiresAt, ban.expiresAt);
