...
export function parseCooldown(value: string): number {
  const match = value.match(/^(\d+)\s*(s|sec|second|seconds|m|min|minute|minutes|h|hr|hour|hours)?$/i);
  if (!match) return 0;
...
export class SlowModeManager {
  ...
  setSlowMode(
    ...
    const slowModeConfig: SlowModeConfig = {
      ...
      bypassRoles: options?.bypassRoles || existingConfig?.bypassRoles || [...this.config.defaultBypassRoles],
      bypassUsers: options?.bypassUsers || existingConfig?.bypassUsers || [],
      maxMessagesPerCooldown: options?.maxMessagesPerCooldown || existingConfig?.maxMessagesPerCooldown || 1,
