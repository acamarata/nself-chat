  ...
  describe('validateReportInput', () => {
    ...
    it('should fail with disabled category', () => {
      const config = {
        ...DEFAULT_REPORT_CONFIG,
        categories: [
          { ...DEFAULT_REPORT_CATEGORIES[0], enabled: false },
        ],
      };
...
describe('ReportQueue Class', () => {
  ...
  describe('getReports', () => {
    beforeEach(() => {
      ...
      queue.createReport(createTestReportInput({
        reporterId: 'reporter-1',
        targetId: 'target-1',
        categoryId: 'spam',
      }));
  ...
  describe('getPendingReports', () => {
    it('should return only pending, in_review, and escalated reports', () => {
      ...
      const { report: report2 } = queue.createReport(createTestReportInput({ targetId: 'target-2' }));
      queue.updateReport(report2!.id, { status: 'resolved' }, 'mod-1');

      const pending = queue.getPendingReports();
      expect(pending.every((r) => ['pending', 'in_review', 'escalated'].includes(r.status))).toBe(true);
    });
  });
...
describe('Factory Functions', () => {
  describe('createReportQueue', () => {
    it('should create queue with default config', () => {
      ...
      expect(queue.getConfig().maxEvidencePerReport).toBe(DEFAULT_REPORT_CONFIG.maxEvidencePerReport);
    });
