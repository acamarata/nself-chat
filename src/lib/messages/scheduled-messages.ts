...
export function calculateNextOccurrence(
  ...
  const { frequency, interval, daysOfWeek, dayOfMonth, endDate, maxOccurrences, sentCount = 0 } = message.recurrence;
 ...
 */
export function sortByScheduledTime(messages: ScheduledMessage[], ascending = true): ScheduledMessage[] {
    ...
    persist(
      subscribeWithSelector(
        immer((set, get) => ({
          ...
          addMessage: (options) => {
            const now = Date.now();
            const scheduledAt = typeof options.scheduledAt === 'number'
              ? options.scheduledAt
          ...
          getMessages: (filter) => {
            ...
            if (filter) {
              ...
              if (filter.fromDate) {
                const fromTime = typeof filter.fromDate === 'number' ? filter.fromDate : filter.fromDate.getTime();
                messages = messages.filter((m) => m.scheduledAt >= fromTime);
              }
              if (filter.toDate) {
                const toTime = typeof filter.toDate === 'number' ? filter.toDate : filter.toDate.getTime();
            ...
            try {
              ...
              return {
                ...
                state: {
                  ...
                  messagesByChannel: new Map(
                    (data.state.messagesByChannel || []).map(([k, v]: [string, string[]]) => [k, new Set(v)])
                  ),
                  messagesByUser: new Map(
                    (data.state.messagesByUser || []).map(([k, v]: [string, string[]]) => [k, new Set(v)])
                  ),
                },
              };
            } catch {
...
export const selectScheduledMessagesForChannel = (channelId: string) => (state: ScheduledMessagesStore) =>
