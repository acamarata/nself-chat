...
export class SessionTracker {
   ...
   * Tracks a page view
   */
  trackPageView(path: string, title: string, options: { referrer?: string; queryParams?: Record<string, string> } = {}): TrackedEvent | null {
    if (!this.session) {
      return null;
    }
    ...
    if (this.config.trackPageViews) {
      return getAnalyticsClient().track(AnalyticsEvent.PAGE_VIEW, {
        ...
        previousPath: this.pageViews.length > 1 ? this.pageViews[this.pageViews.length - 2].path : undefined,
        queryParams: options.queryParams,
      });
    }
