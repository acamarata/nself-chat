  ...
  return bot('faq-bot')
    ...
    .command('removefaq', 'Remove an FAQ', async (ctx, api) => {
      ...
      const index = faqs.findIndex(f =>
        f.id === searchTerm || f.question.toLowerCase().includes(searchTerm.toLowerCase())
      )
    ...
    .onInit(async (bot, api) => {
      ...
      console.log('[FAQBot] Initialized with', storedFAQs?.length || 0, 'FAQs')
    })
...
function extractKeywords(question: string): string[] {
  const stopWords = new Set(['how', 'what', 'when', 'where', 'why', 'who', 'is', 'are', 'the', 'a', 'an', 'do', 'does', 'can', 'could', 'would', 'should', 'i', 'my', 'to'])
...
function isQuestion(message: string): boolean {
  const questionWords = ['how', 'what', 'when', 'where', 'why', 'who', 'can', 'could', 'would', 'should', 'is', 'are', 'does']
