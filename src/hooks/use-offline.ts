...
export function useOffline() {
  ...
  useEffect(() => {
    ...
    syncManager.initialize().catch((error) => {
      console.error('[useOffline] Failed to initialize sync manager:', error);
    });
  ...
  useEffect(() => {
    const updateQueue = async () => {
      try {
        ...
        console.error('[useOffline] Failed to update queue:', error);
      }
    };
  ...
  const clearCache = useCallback(async (): Promise<void> => {
    const { channelStorage, messageStorage, userStorage } = await import('@/lib/offline/offline-storage');
    await Promise.all([
  ...
  useEffect(() => {
    const updateCount = async () => {
      try {
        ...
        console.error('[usePendingCount] Failed to get pending count:', error);
      }
    };
