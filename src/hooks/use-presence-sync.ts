...
export interface UsePresenceSyncReturn {
   ...
   * Set custom status
   */
  setCustomStatus: (status: { emoji?: string; text?: string; expiresAt?: Date | null } | null) => void;
...
export function usePresenceSync(options: UsePresenceSyncOptions = {}): UsePresenceSyncReturn {
  ...
  const resetStaleTimer = useCallback(() => {
    ...
    staleTimerRef.current = setTimeout(() => {
      console.warn('[PresenceSync] Connection appears stale, reconnecting...');
  ...
  const handlePresenceUpdate = useCallback(
    (event: PresenceEvent) => {
      ...
      setUserPresence(event.userId, {
        status: event.status,
        customStatus: event.customStatus || event.customEmoji
    ...
    (event: { presences: PresenceEvent[] }) => {
      ...
      const presences = event.presences
        ...
        .map((p) => ({
          userId: p.userId,
          status: p.status,
          customStatus: p.customStatus || p.customEmoji
  ...
  const handleTypingStart = useCallback(
    (event: { userId: string; channelId: string; threadId?: string; user?: { displayName?: string } }) => {
      if (event.userId === user?.id) return;
