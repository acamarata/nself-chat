...
export class BotsResource extends BaseResource {
  ...
  async create(options: CreateBotOptions): Promise<Bot> {
    return this.post<Bot>('/api/bots', options)
  }
  ...
  async get(botId: UUID): Promise<Bot> {
    return this.get<Bot>(`/api/bots/${botId}`)
  }
  ...
  async list(options?: ListOptions): Promise<PaginatedResult<Bot>> {
    return this.get<PaginatedResult<Bot>>('/api/bots', options)
  }
  ...
  async update(botId: UUID, options: UpdateBotOptions): Promise<Bot> {
    return this.patch<Bot>(`/api/bots/${botId}`, options)
  }
  ...
  async delete(botId: UUID): Promise<void> {
    return this.delete<void>(`/api/bots/${botId}`)
  }
  ...
  async regenerateApiKey(botId: UUID): Promise<{ apiKey: string }> {
    return this.post<{ apiKey: string }>(`/api/bots/${botId}/regenerate-api-key`)
  }
  ...
  async sendMessage(
    ...
    return this.post<Message>(`/api/bots/${botId}/send-message`, options)
  }
  ...
  async addReaction(botId: UUID, messageId: UUID, emoji: string): Promise<void> {
    return this.post<void>(`/api/bots/${botId}/add-reaction`, {
      messageId,
      emoji,
    })
  }
