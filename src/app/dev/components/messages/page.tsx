...
const messageListProps: PropDefinition[] = [
  { name: 'channelId', type: 'string', required: true, description: 'The ID of the channel to display messages for' },
  { name: 'messages', type: 'Message[]', required: true, description: 'Array of messages to display' },
  { name: 'channelName', type: 'string', default: '"general"', description: 'Name of the channel for empty state' },
  { name: 'channelType', type: '"public" | "private" | "dm" | "group-dm"', default: '"public"', description: 'Type of channel' },
  { name: 'isLoading', type: 'boolean', default: 'false', description: 'Show loading skeleton' },
  { name: 'hasMore', type: 'boolean', default: 'true', description: 'Whether there are more messages to load' },
  ...
  { name: 'onLoadMore', type: '() => void | Promise<void>', description: 'Callback when scrolled near top' },
  { name: 'onReply', type: '(message: Message) => void', description: 'Callback when reply is triggered' },
  { name: 'onThread', type: '(message: Message) => void', description: 'Callback when thread is opened' },
  { name: 'onEdit', type: '(message: Message) => void', description: 'Callback when edit is triggered' },
  { name: 'onDelete', type: '(messageId: string) => void', description: 'Callback when delete is triggered' },
  { name: 'onReact', type: '(messageId: string, emoji: string) => void', description: 'Callback for adding reaction' },
]

const messageItemProps: PropDefinition[] = [
  { name: 'message', type: 'Message', required: true, description: 'The message object to render' },
  { name: 'isGrouped', type: 'boolean', default: 'false', description: 'Whether message is part of a group (hides avatar)' },
  { name: 'showAvatar', type: 'boolean', default: 'true', description: 'Whether to show user avatar' },
  { name: 'isCompact', type: 'boolean', default: 'false', description: 'Compact display mode for threads' },
  { name: 'isHighlighted', type: 'boolean', default: 'false', description: 'Highlight the message' },
  ...
  { name: 'onReact', type: '(messageId: string, emoji: string) => void', description: 'Add reaction callback' },
...
const messageInputProps: PropDefinition[] = [
  ...
  { name: 'onSend', type: '(content: string, attachments?: File[]) => void', required: true, description: 'Send callback' },
  { name: 'onTyping', type: '() => void', description: 'Typing indicator callback' },
  { name: 'onEdit', type: '(messageId: string, content: string) => void', description: 'Edit message callback' },
  ...
  { name: 'mentionSuggestions', type: 'MentionSuggestion[]', description: 'User mention suggestions' },
]
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2">
        {['MessageList', 'MessageItem', 'MessageInput', 'MessageSkeleton', 'MessageReactions', 'TypingIndicator', 'MessageSystem'].map((name) => (
        ...
        <TabsContent value="overview" className="space-y-8 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The message system consists of three main components that work together
                to provide a complete chat experience.
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-3 gap-4">
                <PreviewCard title="MessageList">
                  <div className="space-y-2 text-sm">
                    <p className="text-muted-foreground">
                      Virtualized container for messages with infinite scroll, grouping,
  ...
  {messages.map((msg, i) => (
    <div key={msg.id} className="flex gap-3 px-4 py-1 hover:bg-muted/50">
      ...
      <div>
            ...
            <div className="rounded-lg border bg-background">
              ...
              <div className="h-64 overflow-y-auto p-2">
                {mockMessages.map((msg) => (
                  <div
                    key={msg.id}
                    className="group flex gap-3 px-2 py-2 hover:bg-muted/50 rounded"
                  >
                    <div className="h-9 w-9 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-sm font-medium">
                      {msg.user.displayName.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline gap-2">
                        <span className="font-semibold text-sm">{msg.user.displayName}</span>
        ...
        {/* MessageList Tab */}
        <TabsContent value="messagelist" className="space-y-8 mt-6">
          <Card>
            <CardHeader>
              <CardTitle>MessageList</CardTitle>
              <CardDescription>
                A virtualized, infinite-scrolling list of messages with grouping,
                date separators, and typing indicators.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <PreviewGrid cols={2}>
                <PreviewCard title="Virtualization">
                  <p className="text-sm text-muted-foreground">
                    Uses TanStack Virtual to efficiently render large message lists.
                    Only visible messages are rendered in the DOM, enabling smooth
                    scrolling with thousands of messages.
                  </p>
                </PreviewCard>
                ...
                <PreviewCard title="Date Separators">
                  <p className="text-sm text-muted-foreground">
                    ...
                    different days. Shows "Today", "Yesterday", or the full date.
                  </p>
                </PreviewCard>
        ...
        {/* MessageItem Tab */}
        <TabsContent value="messageitem" className="space-y-8 mt-6">
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <CodeBlock code={messageItemCode} language="tsx" filename="message-item-example.tsx" />
            </CardContent>
          </Card>
              ...
              <PreviewGrid cols={2}>
                <PreviewCard title="Default">
                  <div className="flex gap-3 p-2">
                    <div className="h-9 w-9 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white text-sm font-medium">
                      A
                    </div>
                    <div>
                      <div className="flex gap-2 items-baseline">
                ...
                <PreviewCard title="With Reactions">
                  <div className="flex gap-3 p-2">
                    <div className="h-9 w-9 rounded-full bg-gradient-to-br from-green-500 to-emerald-500 flex items-center justify-center text-white text-sm font-medium">
                      B
                    </div>
                    <div>
                      <div className="flex gap-2 items-baseline">
                      ...
                      <div className="flex gap-1 mt-2">
                        <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 border border-primary/20 px-2 py-0.5 text-xs">
                ...
                <PreviewCard title="Compact (Thread)">
                  <div className="flex gap-2 p-2">
                    <div className="h-6 w-6 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-[10px] font-medium">
                      C
                    </div>
                    <div>
                      <div className="flex gap-2 items-baseline">
                        <span className="font-semibold text-xs">Carol</span>
        ...
        {/* MessageInput Tab */}
        <TabsContent value="messageinput" className="space-y-8 mt-6">
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <PreviewGrid cols={2}>
                ...
                <PreviewCard title="@Mentions">
                  <p className="text-sm text-muted-foreground">
                    Type @ to mention users. Suggestions appear as you type
                    with fuzzy matching on display name and username.
                  </p>
                </PreviewCard>
                ...
                <PreviewCard title="Draft Saving">
                  <p className="text-sm text-muted-foreground">
                    Drafts are automatically saved per channel and restored
                    when returning to the channel.
                  </p>
                </PreviewCard>
              </PreviewGrid>
            </CardContent>
