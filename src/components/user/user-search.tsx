...
const UserSearch = React.forwardRef<HTMLDivElement, UserSearchProps>(
  (
    ...
    const displayUsers = React.useMemo(() => {
      let users = filteredUsers.filter((user) => !excludeUserIds.includes(user.id))
                      ...
                      <SelectContent>
                        {presenceOptions.map((presence) => (
                          <SelectItem key={presence} value={presence}>
                            {presence === 'all' ? 'All Statuses' : getPresenceLabel(presence)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {roleFilter !== 'all' && (
              <Badge variant="secondary" className="gap-1">
                ...
                <button onClick={() => setRoleFilter('all')} className="ml-1 hover:text-foreground">
                  <X className="h-3 w-3" />
                </button>
              </Badge>
            )}
          ...
          <div className="space-y-1">
            {displayUsers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">
                  {searchQuery || hasActiveFilters ? 'No users found' : 'Search for users'}
                </p>
              </div>
            ) : (
...
const UserSearchInput = React.forwardRef<HTMLInputElement, UserSearchInputProps>(
