**[Home](Home)**

---

### Getting Started
- [Quick Start](Getting-Started)
- [Installation](Installation)
- [Configuration](Configuration)
- [White-Label Guide](White-Label-Guide)

---

### Features
- [Features Overview](Features)
- [Messaging](Features-Messaging)
- [Channels](Features-Channels)
- [Files & Media](Features-Files)
- [Users & Presence](Features-Users)
- [Real-time](Features-Realtime)
- [Search](Features-Search)
- [Notifications](Features-Notifications)
- [Emoji & Reactions](Features-Emoji)
- [Polls & Voting](Features-Polls)
- [Voice Messages](Features-Voice)
- [Bots & Automation](Features-Bots)

---

### Authentication
- [Auth Overview](Authentication)
- [Email/Password](Auth-Email)
- [Magic Links](Auth-Magic-Link)
- [OAuth Providers](Auth-OAuth)
- [Phone/SMS](Auth-Phone)
- [ID.me Verification](Auth-IDme)
- [Access Control](Auth-Access-Control)

---

### Customization
- [Themes](Themes)
- [Branding](Branding)
- [Feature Flags](Feature-Flags)
- [Landing Pages](Landing-Pages)

---

### Admin
- [Admin Dashboard](Admin-Dashboard)
- [User Management](Admin-Users)
- [Channel Management](Admin-Channels)
- [Analytics](Admin-Analytics)
- [Audit Logs](Admin-Audit)
- [Moderation](Admin-Moderation)
- [Compliance](Admin-Compliance)

---

### Bots & Integrations
- [Bot SDK](Bots)
- [Example Bots](Bots-Examples)
- [Webhooks](Webhooks)
- [Slash Commands](Slash-Commands)
- [Workflows](Workflows)
- [Integrations](Integrations)

---

### API Reference
- [API Overview](API-Reference)
- [GraphQL Queries](API-Queries)
- [GraphQL Mutations](API-Mutations)
- [GraphQL Subscriptions](API-Subscriptions)
- [REST Endpoints](API-REST)

---

### Components
- [Components Overview](Components)
- [UI Components](Components-UI)
- [Chat Components](Components-Chat)
- [Admin Components](Components-Admin)

---

### Hooks
- [Hooks Overview](Hooks)
- [Data Hooks](Hooks-Data)
- [Socket Hooks](Hooks-Socket)
- [UI Hooks](Hooks-UI)

---

### Deployment
- [Deployment Overview](Deployment)
- [Docker](Deployment-Docker)
- [Kubernetes](Deployment-Kubernetes)
- [Vercel](Deployment-Vercel)
- [Desktop Apps](Deployment-Desktop)
- [Mobile Apps](Deployment-Mobile)

---

### Architecture
- [Architecture Overview](Architecture)
- [Project Structure](Project-Structure)
- [Database Schema](Database-Schema)
- [State Management](State-Management)

---

### Plugins
- [Plugin System](Plugins)
- [Available Plugins](Plugins-List)
- [Creating Plugins](Plugins-Creating)

---

### Roadmap
- [v0.2 Planning](Roadmap-v0.2)

---

### Reference
- [Environment Variables](Environment-Variables)
- [TypeScript Types](Types)
- [Contributing](Contributing)
- [Changelog](Changelog)
