...
async function afterPack(context) {
  ...
  try {
    ...
    if (sizeMB > maxSizeMB) {
      log(`WARNING: App size (${sizeMB.toFixed(1)}MB) exceeds recommended maximum (${maxSizeMB}MB)`, 'WARN');
    } else {
      log(`App size is within recommended limits (${sizeMB.toFixed(1)}MB / ${maxSizeMB}MB)`, 'SUCCESS');
    }
